...
const getConversations = async (message, bot) => {
    ...
    let messageHistory = await message.channel.messages.fetch({
        limit: config.messageHistoryLimit,
    });
    messageHistory.reverse();

    messageHistory.forEach((chat) => {
        const senderUsername = chat.author.username
            .replace(/\s+/g, "_")
            .replace(/[^\w\s]/gi, "");
        ...
        if (chat.author.id !== bot.user.id) {
            ...
            if (chat.attachments.size > 0) {
                ...
                const imageAttachments = chat.attachments.filter(
                    (attachment) => {
                ...
                if (imageAttachments.size > 0) {
                    //-- Gets the latest image URL only
                ...
                if (imageURL) {
                    ...
                    conversation.push({
                        ...
                        content: [
                            {
                                type: "text",
                                text: chat.content || "describe this image",
                            },
                            {
                                type: "image_url",
